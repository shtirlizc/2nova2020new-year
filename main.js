+function ($) { "use strict";
    ...
    Form.prototype.prepareVars = function () {
        this.$email = this.$form.find('.js-email');
        ...
        self.$form.on('click', 'input[type="submit"]', function (e) {
            ...
            if (self.$form.valid()) {
                var email = self.$email.val(),
                    isSubscribe = self.$form.find('input#subscribe').prop('checked') ? 1 : 0,
                    nospam = self.$nospam.val(),
                    type = self.$type.val(),
                    params = {email: email, subscribe: isSubscribe, handler: 'onSubscribe', nospam: nospam, type: type};
